...
function ProjectCard({ project, index }) {
  ...
  return (
    <motion.article
      className="group relative h-full rounded-3xl border border-slate-900/10 bg-white/65 p-4 shadow-sm backdrop-blur-xl transition hover:border-electric/40 dark:border-white/10 dark:bg-white/8"
    ...
    >
      ...
      <div
        className={`relative min-h-48 overflow-hidden rounded-[1.25rem] bg-gradient-to-br ${project.accent} p-5 text-white`}
      ...
      >
        ...
        <div className="relative flex h-full min-h-40 flex-col justify-between">
          ...
          <div>
            <p className="text-5xl font-semibold leading-none">{project.metric}</p>
            <p className="mt-2 text-sm font-medium text-white/82">{project.metricLabel}</p>
          </div>
        </div>
      </div>

      <div className="relative p-2 pt-6" style={{ transform: "translateZ(18px)" }}>
        <h3 className="text-xl font-semibold text-graphite dark:text-white">{project.title}</h3>
        ...
        <div className="mt-6 flex items-center gap-3">
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full bg-graphite px-4 py-2 text-sm font-semibold text-white transition hover:-translate-y-0.5 hover:bg-electric dark:bg-white dark:text-ink dark:hover:bg-mint"
          >
            {project.liveLabel}
